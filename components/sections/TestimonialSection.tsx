      ...
      {cards.length > 0 && (
        <StaggerContainer className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          {cards.map((card, index) => {
            ...
            return (
              <StaggerItem key={cardId} className="h-fit self-start">
                <HoverLift>
                  <article className="flex flex-row items-start gap-3 rounded-[2rem] border border-border bg-surface-strong p-3 shadow-lg shadow-black/5 ring-1 ring-black/5 backdrop-blur-sm transition hover:border-accent/40 hover:ring-accent/20 sm:gap-4 sm:p-4 md:p-5">
