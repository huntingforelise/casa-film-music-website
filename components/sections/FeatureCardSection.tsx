    ...
    <SectionShell id="feature-cards">
      ...
      >
        {cards.map((card, index) => {
          ...
          return (
            <StaggerItem key={cardId} className="h-full">
              <HoverLift className="h-full">
                <article className="group h-full overflow-hidden rounded-[2rem] border border-border bg-surface-strong shadow-[0_20px_50px_rgba(0,0,0,0.06)] ring-1 ring-black/5 transition duration-300 hover:border-accent/40 hover:shadow-[0_26px_70px_rgba(0,0,0,0.1)] hover:ring-accent/20">
      ...
      {(calloutTitle || calloutText || calloutItems.length > 0) && (
        <Reveal className="editorial-panel mt-8" delay={0.08}>
